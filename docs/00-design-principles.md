...
**Status:** Foundational. These principles govern the whole `sf-clean-room` tool family. They are garnered from [`01-metadata-ideation.md`](requirements/01-metadata-ideation.md) (v1, metadata) and [`02-data-download.md`](requirements/02-data-download.md) (v2, records), and they sit above both — when a specific design choice is in question, check it against these.
...
### C7. Every change includes and enhances regression testing.
A change to requirements, design, or code is not complete until its tests are. New or altered capability ships **with** tests in the same change — offline (logic, mocked) and, wherever it touches live behaviour, a real check against the configured test org (`tests/live_org.toml`) following [`regression-testing.md`](regression-testing.md). The existing suite must stay green and coverage must not silently narrow: a change that adds behaviour without adding tests, or that leaves a previously-tested path unverified, is incomplete. When a capability is added or changed, **`regression-testing.md` is updated in the same change** so the chatbot-driven test guidance never lags the tool — a new artefact, sentinel, flag, or failure mode that a tester would need to check is documented there as part of the work. Regression testing is part of the deliverable, never a deferred phase; "I'll test it later" is the anti-pattern. This applies with full force to live testing: when the test org is available, run it; when it is not, say so explicitly rather than quietly skipping (B2-adjacent — surface, don't hide).

### C8. The README is the front door and is kept current in the same change.
The root `README.md` is the first thing a human or agent reads, and it must always reflect the tool as it is **today**: near the top, a description of what the system can do and a **complete listing of the available CLI commands with their purposes and sentinels**. Any change that adds, removes, or renames a command, flag, published artefact, sentinel, or consumer-visible behaviour **updates the README in the same change** — a README that advertises fewer (or other) capabilities than the binary ships is a discoverability bug, the same class of defect as help-text drift (C1). The README summarises and points; the design docs remain the authoritative contracts (C6) — but the summary must never be stale.
